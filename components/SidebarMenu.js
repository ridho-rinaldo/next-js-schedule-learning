...
const checkPathName = (pathname, route, hasChildren) => {
  if (pathname === route.path) return true
  else if (hasChildren) {
    return route.insideMenu.some(item => item.path === pathname)
  }
  return false
}
    ...
    <>
      ...
      >
        {appRoutes.map((route, index) => {
          ...
          // const isInsideMenu = route.insideMenu ? true : false;
          if (!hasChildren)
            return (
              <Menu.Item
                key={getKey(route.name, index)}
                className={
                  checkPathName(pathname, route, hasChildren) ? 'ant-menu-item-selected' : ''
                }
                onClick={() => {
